...
class Board(BaseModel):
    ...
    grid: List[List[Optional[str]]] = Field(
        default_factory=lambda: [[None for _ in range(10)] for _ in range(10)]
    )
    ...
    def place_ship(self, ship_name: str, size: int, coords: List[Tuple[int, int]]):
        if len(coords) != size:
            raise ShipPlacementError(
                f"Ship {ship_name} requires {size} coordinates, got {len(coords)}"
            )
        ...
        is_horizontal = all(y == ys[0] for y in ys) and all(
            xs[i + 1] == xs[i] + 1 for i in range(len(xs) - 1)
        )
        is_vertical = all(x == xs[0] for x in xs) and all(
            ys[i + 1] == ys[i] + 1 for i in range(len(ys) - 1)
        )

        if not (is_horizontal or is_vertical) and size > 1:
            raise ShipPlacementError(
                "Ship must be placed in a straight horizontal or vertical line"
            )
        ...
        self.ships.append(
            Ship(name=ship_name, size=size, coordinates=[(x, y) for x, y in coords])
        )
...
class Game(BaseModel):
    ...
    def shoot(self, player_id: str, x: int, y: int) -> Dict:
        ...
        try:
            ...
            raise GameError(
                "Game state inconsistency: turn refers to non-existent player"
            )
        ...
        return {
            ...
            "game_over": False,
            "next_turn": self.players[self.turn],
        }
